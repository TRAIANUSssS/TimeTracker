...
Для приложений дополнительно хранится категория `system`, `user` или `unknown` и версия
локального каталога процессов. При регистрации нового исполняемого файла системная
категория назначается только точному имени в разрешённом каталоге Windows; совпадение
одного имени файла недостаточно. Новые системные процессы создаются с `ignored=1`.
Повторная классификация существующих записей обновляет только категорию и версию каталога,
не меняя выбранный пользователем `ignored`.
