...
- Хотя бы одна единица времени всегда выбрана. Дробные значения имеют максимум
  два знака после запятой; нулевые остатки не выводятся. Очень короткое ненулевое
  время показывается как `<0,01` выбранной единицы. Округление выполняется до
  разложения на единицы, чтобы не появлялись значения вроде `23 ч. 60 м.`.
