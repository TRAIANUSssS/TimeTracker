...
состояния пользователя и статистика по интервалам. Стек — Python, SQLite,
FastAPI и локальный React/TypeScript-интерфейс.
...
Dashboard по референсу, автозапуск из меню трея и переносимая Windows-сборка готовы.

## Запуск готовой сборки

Результат `packaging/build.py` — `dist/TimeTracker-windows-x64.zip`.
...
трекер и API. История хранится отдельно в `%LOCALAPPDATA%\TimeTracker`.
Для обновления сначала выйдите через трей и замените папку приложения целиком.

В **«Настройки сбора»** можно выбрать обычный опрос или экономичный режим ETW.
Для ETW один раз установите фоновый компонент из настроек с подтверждением Windows.
После сохранения режима перезапустите приложение. Обычные запуски и автозапуск
не требуют повышения прав. Установка/обновление/удаление компонента требуют UAC.
Подробности и проверка готовой сборки: [пользовательский ETW](docs/etw-user-mode.md).
...
- [Дорожная карта продукта](docs/product-roadmap.md) — дальнейшие функциональные этапы и приоритеты.
...
├── frontend/                   # React/TypeScript dashboard и браузерные тесты
└── packaging/                  # Windows exe, иконка и сборка ZIP
```
...
Для dashboard из исходников нужен Node.js 22.12+; перед запуском трекера соберите UI:
...
FastAPI раздаёт `frontend/dist` с того же адреса, что и статистику.
...
Левый щелчок открывает dashboard, правый — меню. **«Выход»** завершает запись
и закрывает сессии. Автозапуск включается пунктом меню **«Запускать вместе с Windows»**.
Страница **«Настройки»** содержит сбор активности, отображение, приложения и приватность,
запись и запуск. Доступны поиск приложений, цвета, управление заголовками и видимостью,
формат длительностей, личная граница дня, пауза и автозапуск. Настройки сохраняются
автоматически. Пауза действует до явного возобновления, включая перезапуск приложения;
пропущенное время не дописывается. Расширенная аналитика остаётся заглушкой.
...
Скрипт устанавливает frontend по lockfile, проверяет TypeScript, собирает локальные
ресурсы и шрифты, затем создаёт windowed exe через PyInstaller и ZIP рядом с папкой.
Сборка доступна без подписи установщика, в формате папки с `_internal`.
Подробности и проверка полного приложения: [docs/ui-build.md](docs/ui-build.md).

## Диагностика CPU

Автоматический runner запускает отдельный экземпляр с тестовой БД, сравнивает
нагрузку с диагностикой и без неё и штатно завершает сбор. Режим `--micro`
измеряет только коллектор, `--pid` наблюдает существующий процесс без остановки.
Команды и формат отчётов: [docs/performance.md](docs/performance.md).
