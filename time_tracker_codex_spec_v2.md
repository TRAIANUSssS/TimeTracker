...
Для Modern Standby (S0/AoAc) уведомления desktop suspend/resume могут отличаться
от границ системного сна и включать фоновые пробуждения. Уведомления приостанавливают
сбор; завершённые интервалы уточнять по парам Kernel-Power 506/507 (42/107 для
обычного сна/гибернации) и их `TimeCreated`. Позднее подтверждение атомарно заменяет
пересечение system-state history на SLEEP только внутри текущего открытого run,
сохраняя соседние состояния и обрезая недостоверный foreground. Повторная обработка
не дублирует интервалы. До получения парных событий результат предварительный;
границы по отсутствующим событиям не выдумывать. Старые закрытые runs не переписывать.

При unlock/resume сначала читать быстрый snapshot lock/idle и обновлять состояние
пользователя; затем независимо подтверждать процессы и окно. Медленное чтение metadata
не должно продлевать LOCKED/SLEEP или задним числом заполнять foreground.
...
В реализации UI endpoint также возвращает заголовок `X-TimeTracker-Windows`:
JSON-массив пар `[started_at, ended_at]` выбранных непересекающихся UTC-интервалов.
Это полная шкала выбора, включая будущее и время без записей. Тело ответа остаётся
массивом сегментов. Заголовок позволяет рисовать шкалу по тем же DST-границам,
которые использованы backend, без второго запроса и повторного расчёта в браузере.
