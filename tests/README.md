...
`test_event_benchmark.py`: учёт CPU worker + collector по времени, порядок ABBA,
запрет вывода об экономии по неуспешным прогонам, Windows counters, искусственный
источник в runner и реальные pipe/worker/SQLite при обрыве, пропуске и переполнении
с последующим переподключением. Длительная проверка выполняется отдельно по
[инструкции](../docs/process-events-benchmark.md).
